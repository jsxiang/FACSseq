...
%%
setfig('all');clf
[n,c]=hist(g.mus,round((max(g.mus)-min(g.mus))*10,0));
area(c,n)

% found cutoff to be mu = 8.000
g.fastcleaver=g.mus<8.000;
...
% filter?
muth=g.mus<prctile(g.mus,100); % essentially no filter

[l1l2seqs,l1l2,ttl]=findSeqs(g,l1len,l2len,endslen,muth);
...
feats{end+1}='cleaver';
fid= fopen('/Volumes/smolke-lab$/Joy/YFSI/N7_end3-3_bitmat_classify.csv','w');
csvtitle=sprintf('%s,',feats{:});
fprintf(fid,'%s\n',csvtitle);

for i=1:length(l1l2)
    s=sprintf('%0.0f,',bitmat(i,:));
    ss=sprintf('%s%0.0f',s,g.fastcleaver(l1l2(i)));
    fprintf(fid,'%s\n',ss);
end
fclose(fid);
